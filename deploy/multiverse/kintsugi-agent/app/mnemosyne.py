...
class MnemosyneManager:
    ...
    async def initialize(self):
        ...
        async with self.engine.begin() as conn:
            ...
            """))
            # Create index for SIRA retrieval
            await conn.execute(sa.text("""
    ...
    async def consolidate(self) -> dict:
        ...
        async with self.session_factory() as session:
            # Decay significance of old low-significance interactions
            result = await session.execute(
                sa.text("""
                    UPDATE interactions
    ...
    def _score_significance(self, user_msg: str, assistant_msg: str) -> float:
        ...
        # Length heuristic — longer exchanges tend to be more significant
        ...
        # Emotional/urgent markers
        urgent_words = {"help", "urgent", "emergency", "crisis", "need", "please"}
        if any(w in user_msg.lower() for w in urgent_words):
            score += 0.15

        # Aid-specific markers
        aid_words = {"resource", "benefit", "housing", "food", "rent", "medical", "childcare"}
        if any(w in user_msg.lower() for w in aid_words):
            score += 0.1
